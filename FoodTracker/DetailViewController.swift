...
class DetailViewController: UIViewController {
    ...
    func createAttributedString (usdaItem: USDAItem!) -> NSMutableAttributedString {
        ...
        var leftAllingedParagraphStyle = NSMutableParagraphStyle()
        leftAllingedParagraphStyle.alignment = NSTextAlignment.Left
        leftAllingedParagraphStyle.lineSpacing = 20.0
        
        var styleFirstWordAttributesDictionary = [
            NSForegroundColorAttributeName: UIColor.blackColor(),
            NSFontAttributeName: UIFont.boldSystemFontOfSize(18.0),
            NSParagraphStyleAttributeName: leftAllingedParagraphStyle]
    ...
    func requestAuthorizationForHealthStore() {
        ...
        store.healthStore.requestAuthorizationToShareTypes(NSSet(array: dataTypesToWrite), readTypes: NSSet(array: dataTypesToRead), completion: { (success, error) -> Void in
            ...
            if success {
                println("user completed authorization success")
            }
            else {
